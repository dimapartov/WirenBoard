...
defineRule("kitchenMotion", {
    ...
    then: function (newValue, devName, cellName) {
        if (dev["KitchenLed_virt/autoEn"] == true) {
            ...
            if (newValue == "true") {
              	var intervalCheck = IsWithinAllowedInterval(dev["KitchenLed_virt/autoBeginHH"], dev["KitchenLed_virt/autoBeginMM"], dev["KitchenLed_virt/autoEndHH"], dev["KitchenLed_virt/autoEndMM"]);
                if (intervalCheck == true || dev["KitchenLed_virt/roundTheClock"] == true) {
