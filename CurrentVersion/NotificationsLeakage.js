    ...
    then: function (newValue, devName, cellName) {
        ...
        if (newValue == "true" || newValue == true) {
            ...
            if (devName == "wb-mwac_203") {
              SendTelegramMsg(1, "АВАРИЯ! Утечка - проводной датчик (МСУ/Прихожая)");
            } else {
                if (devName == "wb-gpio") { // Сработал дискретный вход, на который сухим контактом подключен модуль MWAC в ЩР-8.2 (ниша БСУ)
                  SendTelegramMsg(1, "АВАРИЯ! Утечка - проводной датчик (БСУ)");
                } else {
        ...
        if (newValue == "true" || newValue == true) {
            if (dev["Smoke sensor (шкаф в прихожке)/smoke"] != "true") {
                ...
                dev["wb-mwac_203/K2"] = false;
              	SendTelegramMsg(1, "Вода перекрыта!");
            }
        }
    }
});